...
class SAGEPairwiseInference():
    def __init__(self, infer_model, input_path, output_path, para_num, temperature=0.9, num_iterations=5):
    ...
    def _infer_one(self, task_with_iteration):
        ...
        try:
            ...
            # print(f"iteration: {iteration}, 系统指令长度: {len(system_instruction)}")
    ...
    def _infer_parallel(self, tasks, para_num):
        ...
        def process_and_save(entry):
            ...
            if all(task_results[task_idx]):
                ...
                self._save_result_realtime(formatted_item)
                # 清理已保存的数据以节省内存
        ...
        for task_idx, pair_results in task_results.items():
            if all(pair_results):
                ...
                remaining_results.append(formatted_item)
                self._save_result_realtime(formatted_item)
        
        print(f"实时保存完成，总共保存了 {self.saved_count} 条数据")
        return results
    ...
    def _save_result_realtime(self, formatted_item):
        """实时保存单个格式化后的数据项到JSONL文件"""
        try:
            with self.file_lock:
                # 确保输出目录存在
                ...
                self.saved_count += 1
                if self.saved_count % 100 == 0:  # 每100条打印一次进度
                    print(f"已保存 {self.saved_count} 条数据到 {self.output_path}")
    ...
    def __call__(self):
        ...
        # 注意：这里不再需要_result_format和_save_result，因为已经在_infer_parallel中实时处理了
        ...
        print(f"**** 结果已实时保存到: {self.output_path} ****")
