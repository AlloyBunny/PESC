...
parser.add_argument(
    ...
    choices=["personalized", "global", "none"],
    help="PRT_TYPE可选值：personalized, global, none（分别为：使用该用户自身的PRT、使用全局用户画像、不启用PRT）",
)
parser.add_argument(
    "--prt_levels",
    type=str,
    default="L3,L2,L1",
    help="PRT_LEVELS：仅在PRT_TYPE=personalized的情况下生效。支持 L3、L2、L1 三个选项的任意组合（用逗号分隔），顺序不影响结果",
)
parser.add_argument(
    "--user_llm_type",
    type=str,
    default="deepseek",
    help="USER_LLM_TYPE：用户（玩家）使用的LLM类型，可选值：gpt, deepseek, gemini, local",
)
parser.add_argument(
    "--assistant_llm_type",
    type=str,
    default="gpt",
    help="ASSISTANT_LLM_TYPE：助手（NPC）使用的LLM类型，可选值：gpt, deepseek, gemini, local",
)
parser.add_argument(
    "--base_url",
    type=str,
    default="none",
    help="LOCAL_MODEL_BASE_URL：本地模型服务的URL",
)
parser.add_argument(
    "--num_threads",
    type=int,
    default=10,
    help="工作线程数（并发处理的simulator数量），默认10",
)

parser.add_argument("--HOST", type=str, default=None, help="HOST：API服务的HOST")
parser.add_argument("--user_model_market", type=str, default=None, help="USER_MODEL_MARKET：用户侧API模型")
parser.add_argument("--user_USER_NAME", type=str, default=None, help="USER_USER_NAME：用户侧API服务的USER_NAME")
parser.add_argument("--user_USER_TOKEN", type=str, default=None, help="USER_USER_TOKEN：用户侧API服务的USER_TOKEN")
parser.add_argument("--assistant_model_market", type=str, default=None, help="ASSISTANT_MODEL_MARKET：助手侧API模型")
parser.add_argument("--assistant_USER_NAME", type=str, default=None, help="ASSISTANT_USER_NAME：助手侧API服务的USER_NAME")
parser.add_argument("--assistant_USER_TOKEN", type=str, default=None, help="ASSISTANT_USER_TOKEN：助手侧API服务的USER_TOKEN")
...
def process_simulator(simulator: dict):
    ...
    while testmode != "human":
        ...
        if turns == 1:
            ...
            if (
                "Goodbye" in player["history"][-1]["content"]
                or "Bye" in player["history"][-1]["content"]
            ):
                break
